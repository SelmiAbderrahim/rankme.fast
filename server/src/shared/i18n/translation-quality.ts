...
export const BRAND_TOKENS = [
    ...
    'MRR', // monthly-recurring-revenue acronym (superadmin metric)
...
export const ALLOWLIST_KEY_PATTERNS: RegExp[] = [
    ...
    /(^|\.)plan\.(free|starter|basic|pro|business|agency|scale|enterprise)$/i, // tier proper nouns
...
const isAllowlistedKey = (key: string): boolean => ALLOWLIST_KEY_PATTERNS.some((re) => re.test(key));
/**
 * Classify a single (en → locale) leaf pair. Returns the failing reason, or
 * null when the value is acceptable.
 */
export const classifyValue = (locale: string, namespace: string, key: string, enValue: string, value: string): QualityReason | null => {
    if ((ALLOWLIST_NAMESPACES as readonly string[]).includes(namespace))
        return null;
    if (isAllowlistedKey(key))
        return null;
    if (MARKUP_RE.test(value))
        return null; // literal markup / code snippet
    const stripped = stripPlaceholders(value).trim();
    if (stripped === '')
        return null; // placeholder-only value (e.g. "{{count}}")
    ...
    if ((NON_LATIN_LOCALES as readonly string[]).includes(locale) && isPureAscii(value)) {
        if (LATIN_WORD.test(stripAllowed(stripped)))
            return 'ascii-latin';
    }
...
export const collectQualityIssues = (locale: string, namespace: string, enTree: unknown, localeTree: unknown): QualityIssue[] => {
